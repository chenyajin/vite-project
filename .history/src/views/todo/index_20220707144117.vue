 ...
 * @LastEditTime: 2022-07-07 14:41:00
 * @Description: 
-->
<template>
  <div class="todo-wrapper">
    <h2>TO DO LIST</h2>
    <el-input v-model="keyword" @keyup.enter="onAdd" ref="inputRef" />
    <div class="list-wrapper">
      <p v-for="item in list" :class="[item.complete ? 'complete-text' : '']" :key="item">
        <el-icon
          v-if="item.complete"
          @click="onComplete(item, false)"
          class="complete-color cursor-pointer"
          size="18"
          ><CircleCheckFilled
        /></el-icon>
        <el-icon v-else @click="onComplete(item, true)" class="cursor-pointer"
          ><CircleCheck
        /></el-icon>
